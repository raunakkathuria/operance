...
usage() {
    cat <<'EOF'
Usage: scripts/uninstall_systemd_user_service.sh [options]

Disable and remove the repo-local Operance tray user service.
...
if [[ "${skip_systemctl}" -eq 0 ]]; then
    echo "+ systemctl --user disable --now operance-tray.service || true"
    if [[ "${dry_run}" -eq 0 ]]; then
        systemctl --user disable --now operance-tray.service >/dev/null 2>&1 || true
    fi
fi
