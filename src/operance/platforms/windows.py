...
WINDOWS_ADAPTER_BLOCKER = (
    "Windows desktop adapter is not implemented yet, so live desktop commands "
    "are blocked on this platform."
)

WINDOWS_CHECK_METADATA = (
    CheckMetadata("windows_platform", "Windows platform", required_for_local_runtime=True),
    CheckMetadata(
        "windows_desktop_adapter",
...
class WindowsDesktopPlatformProvider:
    ...
    def build_adapters(self, config) -> AdapterSet:
        return build_blocked_adapter_set(blocker=WINDOWS_ADAPTER_BLOCKER)
        ...
        return [
            {
                ...
                "status": "ok" if system_name == "Windows" else "warn",
                "detail": system_name,
            },
            {
                "name": "windows_desktop_adapter",
                "status": "warn",
                "detail": "Windows desktop adapter is not implemented yet.",
            },
        ]
    ...
    def tool_live_runtime_blockers(
        ...
        return ["Windows desktop adapter"]
