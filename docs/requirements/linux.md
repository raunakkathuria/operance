...
- planner prompt generation now includes per-tool required-arg hints, confirmation and risk hints, plus example transcripts, and the planner payload schema now uses per-tool argument contracts instead of accepting free-form model arguments
