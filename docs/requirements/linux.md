...
The verified deterministic surface now also accepts safe everyday variants such as `search google for <query>`, `search the web for <query>`, `open downloads`, `open folder downloads`, `open documents`, `open desktop`, `list files in downloads`, `show files in documents`, `find file named <name>`, `find folder named <name>`, `search documents for <name>`, `time`, `battery`, `volume`, `muted`, `volume 50 percent`, `mute`, and `unmute`. Folder-open commands remain semantic `files.open` actions, and read-only file discovery remains semantic `files.list_folder` or `files.find` actions. Linux resolves known folders through the file adapter, skips hidden paths, bounds search results, and never executes arbitrary shell commands or accepts raw filesystem paths from speech.
