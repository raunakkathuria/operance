...
The CLI now also exposes `python3 -m operance.cli --getting-started`, which prints the current first-run activation path, a readiness checklist, packaged-install readiness when running the RPM, click-to-talk smoke commands, optional local AI planner validation state, support-bundle guidance, and contributor next steps without executing desktop actions. The tray exposes this same product-facing guidance as **First run setup** so installed beta users can validate the supported path without memorizing CLI diagnostics. The same safe source-checkout and installed-package smoke scripts run this diagnostic so onboarding regressions fail before release.
