...
The CLI now also exposes `python3 -m operance.cli --getting-started`, which prints the current first-run activation path, what to try, local AI planner state, and contributor next steps without executing desktop actions. The same safe source-checkout and installed-package smoke scripts run this diagnostic so onboarding regressions fail before release.
