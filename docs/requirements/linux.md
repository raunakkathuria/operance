...
On Linux, `open browser`, `open web browser`, and `open default browser` now resolve through the adapter to the system default browser instead of trying to launch a literal `browser.desktop` entry. Website phrases such as `open google.com` are treated as browser navigation and normalized to `https://google.com` before execution. Installed app names are resolved inside the Linux adapter from desktop-entry metadata such as desktop file id, `Name`, `GenericName`, `Keywords`, and `Exec` basename before falling back to the older executable or `gtk-launch <name>.desktop` path.
