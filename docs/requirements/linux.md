...
When the always-on trigger fires, the tray now shows an `I'm listening` acknowledgement before the command phrase is captured. If the experimental sound trigger fires but no command follows before the bounded command window expires, the loop records `Sound trigger detected, but no command followed.`, returns to waiting, and shows that warning in the tray instead of staying stuck in command-listening state. With a configured model-backed wake-word detector, phrase-aware no-command feedback is allowed; `Hey Ops` is the preferred future public wake phrase, but the current package does not bundle that model.
