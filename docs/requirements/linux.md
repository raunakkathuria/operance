...
Always-on listening is wake-word gated and should still be tested first as `Operance`, a short pause, then the command, for example `open browser`. The live path also supports one continuous phrase such as `Operance open browser` by feeding the wake frame into STT before command parsing. Click-to-talk remains more responsive because it starts STT immediately after the tray click, while the continuous voice loop still depends on wake detection first.
When the always-on wake word is detected, the tray now shows an `I'm listening` acknowledgement before the command phrase is captured. If the wake word is heard but no command follows before the bounded command window expires, the loop records `I heard Operance, but no command followed.`, returns to wake waiting, and shows that warning in the tray instead of staying stuck in command-listening state.
