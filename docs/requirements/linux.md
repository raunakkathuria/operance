...
The verified deterministic surface now also accepts safe everyday variants such as `search google for <query>`, `search the web for <query>`, `open downloads`, `open folder downloads`, `open documents`, `open desktop`, `time`, `battery`, `volume`, `muted`, `volume 50 percent`, `mute`, and `unmute`. Folder commands remain semantic `files.open` actions; Linux resolves the known folder through the file adapter and opens it with `xdg-open`, while the core never executes arbitrary shell commands or accepts raw filesystem paths from speech.
