...
Always-on listening is wake-word gated and should be tested as `Operance`, a short pause, then the command, for example `open browser`. This differs from click-to-talk: click-to-talk starts STT immediately after the tray click, while the continuous voice loop starts STT only after wake detection. Saying `Operance open browser` as one continuous phrase may be less reliable with the current fallback wake-word path, especially before a tuned model-backed wake word is configured.
