...
- `search documents for <name>`
- `show details for <name>`
- `how big is <name>`
- `when was <name> modified`
...
The default tray menu is now kept end-user focused: click-to-talk, always-on listening, supported commands, setup/status, issue reporting, recent interaction details when available, update checks, about, confirmation/undo when relevant, and quit. Raw planner readiness, installed-smoke diagnostics, support snapshots, and service-reset controls remain available through CLI commands or setup/status flows instead of crowding the default right-click menu.
The tray `Report an issue` action writes the preferred redacted `tar.gz` support-bundle artifact to `data_dir/support-bundles/` without requiring a separate shell command.
Tray-saved support-bundle artifacts now also include the current Operance version in their generated filenames, so files collected from multiple builds do not collapse into ambiguous timestamp-only names.
When that heartbeat is stale, setup/status and CLI diagnostics still point at the shared voice-loop service restart path; the default tray menu keeps service-reset controls out of the normal user flow.
...
The tray menu now also exposes always-on listening controls backed by the existing voice-loop user service. Click-to-talk remains the default reliable path, while the start/stop control lets a configured voice-loop runtime be managed without copying `systemctl` commands.
Always-on listening now distinguishes model-backed wake-word mode from the built-in experimental sound-trigger fallback. Without a model-backed asset, test the fallback first as `Operance`, a short pause, then the command, for example `open browser`; that trigger is based on sustained audio energy, not true phrase recognition. The live path also supports one continuous phrase such as `Operance open browser` by feeding the trigger frame into STT before command parsing. Click-to-talk remains more responsive because it starts STT immediately after the tray click, while the continuous voice loop still depends on trigger detection first.
When the always-on trigger fires, the tray now shows an `I'm listening` acknowledgement before the command phrase is captured. If the experimental sound trigger fires but no command follows before the bounded command window expires, the loop records `Sound trigger detected, but no command followed.`, returns to waiting, and shows that warning in the tray instead of staying stuck in command-listening state. If STT captures only a clipped tail fragment such as `is it?`, the loop records an incomplete-command recovery response and asks the user to repeat the full command after `Operance` instead of sending the fragment through normal command matching. After a completed always-on command, the loop also applies a short post-command trigger suppression window so the tail of the same utterance is less likely to be treated as a new trigger. With a configured model-backed wake-word detector, phrase-aware no-command feedback is allowed; `Hey Ops` is the preferred future public wake phrase, but the current package does not bundle that model.
After a no-command wake, the continuous loop suppresses wake detection for a
short cooldown and the energy fallback requires a longer sustained activation
streak. This keeps coughs or brief noise from stacking repeated no-command tray
warnings while preserving the same typed command path after a real command is
captured.
Always-on command results take priority over the wake acknowledgement once a final transcript has produced a response, so users see the outcome instead of a stale `I'm listening` bubble. Tray bubbles use readable timeouts rather than very short flashes, so users have time to see listening, warning, and result state changes.
For source-checkout tray testing, the always-on tray control now starts and stops a repo-local `python -m operance.cli --voice-loop` child process with the same environment instead of controlling the packaged systemd voice-loop service. Packaged installs still use `systemctl --user` for `operance-voice-loop.service`. This keeps no-reinstall testing aligned on one data directory and avoids comparing a source tray against an installed voice-loop runtime.
The wake-word-gated live path now trims short leading wake-word residue before known command starters, so common STT outputs such as `Operants, open browser` are normalized to `open browser` before entering the daemon. This cleanup is scoped to the always-on wake-word path and does not change click-to-talk transcription or the core typed action model. Incomplete-fragment recovery is also scoped to the always-on capture path and is deliberately conservative; Operance does not infer that a fragment such as `is it?` means `what time is it`.
Manual voice sessions now also produce a short spoken-response text payload for success, no-transcript, unmatched, failure, and confirmation-required results. The tray records that phrase in the last-interaction report even when TTS assets are not installed, and actual audio playback still depends on the existing optional TTS configuration.
...
On Linux, `open browser`, `open the browser`, `open web browser`, and `open default browser` now resolve through the adapter to the system default browser instead of trying to launch a literal `browser.desktop` entry. Website phrases such as `open google.com`, `go to google.com`, `visit docs.python.org/3`, and `open website github.com/raunakkathuria/operance` are treated as browser navigation and normalized to `https://...` before execution. Installed app names are resolved inside the Linux adapter from desktop-entry metadata such as desktop file id, `Name`, `GenericName`, `Keywords`, and `Exec` basename before falling back to the older executable or `gtk-launch <name>.desktop` path.
...
The verified deterministic surface now also accepts safe everyday variants such as `search google for <query>`, `search the web for <query>`, `open downloads`, `open folder downloads`, `open documents`, `open desktop`, `list files in downloads`, `show files in documents`, `find file named <name>`, `find folder named <name>`, `search documents for <name>`, `show details for <name>`, `how big is <name>`, `when was <name> modified`, `show recent downloads`, `copy file on desktop called <name> to documents`, `open it`, `open the first one`, `copy the first one to documents`, `switch to it`, `switch to the first window`, `what can I say`, `what did you hear`, `are you listening`, `is local AI ready`, `why did that fail`, `what apps are open`, `is <app> open`, `find window <title>`, `switch to <title> window`, `time`, `battery`, `volume`, `muted`, `volume 50 percent`, `mute`, and `unmute`. Folder-open commands remain semantic `files.open` actions, read-only file discovery remains semantic `files.list_folder` or `files.find` actions, file metadata remains semantic `files.get_info` or `files.list_recent_folder` actions, known-folder entry copy remains a semantic `files.copy` action with no-overwrite behavior, contextual follow-ups are resolved from the daemon's previous semantic file or window references into existing typed actions, runtime self-status questions are handled by the portable daemon without OS adapter execution, and open-window checks remain semantic `windows.find` actions. Linux resolves known folders through the file adapter, skips hidden paths, bounds search results, and never executes arbitrary shell commands, reads file contents, or accepts raw filesystem paths from speech.
Contextual follow-ups are intentionally strict: `open it` and `open the first one` only resolve against fresh file results, `copy it to documents` and `copy the first one to downloads` only resolve against fresh file results and known-folder destinations, while `switch to it` and `switch to the first window` only resolve against fresh window results. Missing, incompatible, or multi-result context returns a recovery message instead of guessing. When a follow-up resolves, status snapshots, transcript JSON, and tray last-interaction details include an `Understood: ...` explanation such as `Open downloads item: alpha.txt` or `Copy downloads item alpha.txt to documents`.
Selected-file copy from the active file manager is intentionally deferred. The current Linux beta must not implement that path by simulating shortcuts, mutating the clipboard to infer file URIs, or scraping Dolphin/KWin state. It can graduate only after a provider exposes a reliable typed selected-file capability with clear blockers, user feedback, validation, and release evidence.
While a tray command is in flight, the product surface now acknowledges the command before completion: it shows `Understanding command` after a transcript is received, then `Opening <target>` for launch-style commands or `Executing command` for other actions. The tooltip includes `Heard: ...` until the final response is available, so users can tell whether Operance heard them and started work.
...
When a command is unmatched, the shared response now gives concrete next examples instead of only reporting failure: `open browser`, `open google.com`, `search google for linux automation`, and `what time is it`. The supported-command tray help and public beta smoke checklist use the same safe examples where the backing tools are available. Known Firefox speech-to-text variants such as `fire fall`, `fire force`, and `fire fox` continue to be corrected only inside semantic app targets before typed action validation and adapter execution.
The tray now also includes `Try commands`, a guided command coach for first-run users. It is backed by the same portable payload as `operance --command-coach`, showing what to say, what should happen, and when to use `Report an issue` if a listed command fails.
The tray now also includes `Beta feedback guide`, backed by the same 10-minute tester loop as `operance --beta-feedback`, so public beta users can see install, verify, try, and report steps without reading raw diagnostics.
The tray setup/status flow includes the same installed-package diagnostic as `operance --installed-smoke` and renders check failures, warnings, next-step commands, and manual click-to-talk checks from the product surface.
Installed smoke, public beta checklist, and tray setup/status now also expose one product-facing `next_action`, so ready installs lead with `Click the tray icon once, then say: open browser` while degraded installs point at the first readiness command to run.
...
The packaged tray shows `Check for updates` and setup/status guidance, so users can inspect release-channel status, readiness, and next steps after install without finding the CLI command first.
...
The CLI now also exposes `python3 -m operance.cli --getting-started`, which prints the current first-run activation path, a readiness checklist, packaged-install readiness when running the RPM, click-to-talk smoke commands, optional local AI planner validation state, support-bundle guidance, and contributor next steps without executing desktop actions. The tray exposes this same product-facing guidance as **Setup and status** so installed beta users can validate the supported path without memorizing CLI diagnostics. The same safe source-checkout and installed-package smoke scripts run this diagnostic so onboarding regressions fail before release.
The CLI now also exposes `python3 -m operance.cli --local-ai-coach`, and the tray exposes the same product-facing guide as **Local AI setup**. This wraps the existing planner setup and status surfaces into a safer first-run flow: start an Ollama-compatible local server outside Operance, export endpoint/model variables, run readiness, run one explicit `--planner-execute` test, and enable live fallback only after readiness passes. It does not install model runtimes, pull models, start servers, or enable planner fallback automatically.
The CLI now also exposes `python3 -m operance.cli --planner-setup-template`, which prints non-mutating setup templates for generic OpenAI-compatible servers, llama.cpp, and Ollama. The templates include endpoint and model environment variables, validation commands, and the planner safety contract, but they do not install model runtimes, download model assets, start servers, or enable live planner fallback.
The CLI now also exposes `python3 -m operance.cli --planner-status`, which prints non-executing local planner state, readiness next steps, and the safety contract that keeps model output bounded to typed actions. Use `--planner-status` for quick local AI setup inspection and `--planner-readiness` when you need a health plus non-executing smoke check against a running model endpoint.
The CLI now also exposes `python3 -m operance.cli --supported-commands`, which projects the typed command catalog with user-facing groups, example transcripts, current live blockers from doctor/setup state, and release-verification status. Use that when a developer needs to answer both “what can I say?” and “why is this command not live on this machine?” from one surface. When a tester only needs the current verified subset, `python3 -m operance.cli --supported-commands --supported-commands-available-only` filters the catalog down to commands that are both live on the current machine and release-verified for the Fedora KDE developer target. That filtered view includes the first confirmation-gated app command, `quit <app name>`, basic audio set or mute commands, the verified desktop file command batch, and the verified `list windows` or `switch to window <title>` window commands after they have been live-verified. It remains the preferred next-step path from setup, and the repo-local MVP wrapper can print it through `./scripts/run_mvp.sh --supported-commands --supported-commands-available-only`, so developers can reach the same conservative discovery path from the main bring-up flow instead of remembering a separate raw CLI flag.
...
python3 -m operance.cli --setup-run-action check_planner_readiness --setup-dry-run
...
- media control accepts the most common playback phrases such as `pause`, `play`, `play music`, `resume music`, `next track`, `next song`, `skip song`, `previous track`, and `go back a track`, mapped to typed `media.play_pause`, `media.next`, and `media.previous` actions. Linux resolves the active player over the MPRIS session-bus interface through `gdbus`, so it works with any MPRIS-compatible player such as VLC, Spotify, or a browser rather than special-casing one app. When no player is running, the command fails with a clear message instead of silently doing nothing. These commands remain outside the release-verified subset until their live Fedora KDE Wayland evidence is captured
- window minimize, maximize, restore, and close now accept natural phrasing beyond the original `<verb> window <title>` form, including `minimize firefox`, `minimize the firefox window`, `maximize the firefox window`, `restore firefox`, `unminimize firefox`, and `close the firefox window`, plus the British `minimise`/`maximise` spellings. `close firefox` is intentionally left unmatched, because it is ambiguous between closing one window and quitting the app, and it falls through to the usual recovery guidance instead of guessing. These commands remain outside the release-verified subset until their live Fedora KDE Wayland evidence is captured through the manual window checks in `operance --installed-smoke`
...
- the repo now also includes `./scripts/run_checkout_smoke.sh`, which runs `--version`, `--doctor`, `--setup-actions`, the runnable-only supported-command subset, the planner MVP fixture, and `--support-bundle` in one fail-fast sequence for Linux contributor bring-up and issue reproduction
...
- when that repo-local heartbeat is stale, setup/status and CLI diagnostics expose the direct restart path for `operance-voice-loop.service` without keeping that service-reset action in the default tray menu
...
- long-lived MCP sessions can now also clear planner cooldown and recent planner-error state without restarting the daemon when the local planner endpoint recovers; the default tray menu leaves that reset action to diagnostics instead of exposing it as a normal user control
...
- planner fallback can call a local OpenAI-compatible chat-completions endpoint, such as a Qwen-class model served through llama.cpp, when `OPERANCE_PLANNER_ENABLED=1` and the transcript is final, unmatched, and above the configured confidence threshold
- planner prompt generation now includes per-tool required-arg hints, confirmation and risk hints, plus example transcripts, while the full planner payload schema stays in the chat-completions request instead of being duplicated into the prompt text
- planner output is still parsed into typed actions, validated against the action registry, policy-checked, and routed through confirmation gates before any Linux adapter can execute it
- local planner onboarding should start with `python3 -m operance.cli --planner-setup-template`, then an OpenAI-compatible server that exposes `/v1/chat/completions` and either sibling `/v1/models` or `/health`; set `OPERANCE_PLANNER_ENDPOINT`, `OPERANCE_PLANNER_MODEL`, `OPERANCE_PLANNER_TIMEOUT_SECONDS`, and `OPERANCE_PLANNER_MAX_RETRIES`, then run `python3 -m operance.cli --planner-readiness "open firefox and notify me"` before setting `OPERANCE_PLANNER_ENABLED=1`
- the Ollama setup template defaults to `qwen2.5:3b` with a longer readiness timeout, which keeps the current local planner smoke closer to a CPU-first developer laptop path without changing the generic OpenAI-compatible server defaults
...
- support snapshots now include a planner-readiness summary derived from doctor checks, while the full planner-readiness health plus smoke report remains available through the CLI
