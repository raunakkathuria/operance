...
- Verified command subset on Fedora KDE Wayland: `open browser`, `open the browser`, `open google.com`, `go to <website>`, `search google for <query>`, `search the web for <query>`, `open <app name>` for installed desktop apps, safe two-step launch phrases such as `open firefox and load localhost:3000` or `open firefox and notify me`, `focus <app name>`, confirmation-gated `quit <app name>`, `open downloads`, `open folder downloads`, `open documents`, `open desktop`, `show recent files`, read-only known-folder discovery such as `list files in downloads`, `show files in documents`, `find file named <name>`, `find folder named <name>`, and `search documents for <name>`, read-only metadata commands such as `show details for <name>`, `how big is <name>`, `when was <name> modified`, and `show recent downloads`, contextual follow-ups such as `open it`, `open the first one`, `copy the first one to documents`, `switch to it`, and `switch to the first window` after compatible file or window awareness results, runtime self-status questions such as `what can I say`, `what did you hear`, `are you listening`, `is local AI ready`, and `why did that fail`, `create folder on desktop called <name>`, safe known-folder entry copy commands such as `copy file on desktop called <name> to documents`, confirmation-gated desktop file or folder delete, rename, and move commands, window awareness commands such as `list windows`, `what apps are open`, `is <app> open`, `find window <title>`, `switch to window <title>`, and `switch to <title> window`, `show a notification saying <message>`, `what time is it`, `time`, `what is my battery level`, `battery`, `wifi status`, `what is the volume`, `volume`, `is audio muted`, `muted`, `set volume to 50 percent`, `volume 50 percent`, `mute audio`, `mute`, `unmute audio`, and `unmute`.
- Tray feedback: when a command is heard, the tray state changes through `Understanding command` and `Opening <target>` or `Executing command`, and the tooltip shows `Heard: ...` before the final result notification. Last interaction details include `Understood: ...` when Operance can explain the typed action it chose.
- Command recovery: unclear commands now return concrete examples such as `open browser`, `open google.com`, `search google for linux automation`, and `what time is it`; unclear contextual follow-ups explain whether a prior file or window result is missing or ambiguous; known Firefox speech variants such as `fire fall` and `fire force` are conservatively mapped to `firefox` for launch, focus, and quit commands.
...
- Contextual follow-ups: after file or window awareness results, users can say `open it`, `open the first one`, `copy it to documents`, `copy the first one to downloads`, `switch to it`, or `switch to the first window`; Operance resolves those phrases into existing typed actions, shows what the reference resolved to, refuses ambiguous or missing context instead of guessing, and still runs validation and policy before execution.
