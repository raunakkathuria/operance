...
def test_live_command_smoke_dry_run_prints_controlled_fixture_steps() -> None:
    ...
    assert result.stdout.splitlines() == [
        ...
        '+ mkdir -p "${tmp_dir}/Documents" "${tmp_dir}/Downloads"',
        '+ write ${tmp_dir}/.config/user-dirs.dirs',
        ...
        '+ OPERANCE_DEVELOPER_MODE=0 .venv/bin/python -m operance.cli --desktop-dir ${tmp_dir}/Desktop --transcript "copy file on desktop called notes.txt to documents"',
        '+ test -f "${tmp_dir}/Documents/notes.txt"',
        '+ test -f "${tmp_dir}/Desktop/notes.txt"',
        '+ rm -f "${tmp_dir}/Documents/notes.txt"',
        '+ touch "${tmp_dir}/Downloads/alpha.txt"',
        '+ write ${tmp_dir}/copy-followup.txt',
        '+ OPERANCE_DEVELOPER_MODE=0 .venv/bin/python -m operance.cli --desktop-dir ${tmp_dir}/Desktop --transcript-file ${tmp_dir}/copy-followup.txt',
        '+ test -f "${tmp_dir}/Documents/alpha.txt"',
        '+ test -f "${tmp_dir}/Downloads/alpha.txt"',
        '+ rm -f "${tmp_dir}/Documents/alpha.txt"',
